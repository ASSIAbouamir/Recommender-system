...
def initialize_system(
    ...
    if Path(processed_data_path).exists():
        ...
        # Try all possible review filenames
        possible_files = [
            Path(data_path) / "C:/Users/user1/Desktop/RecLM-RAG/data/sample/reviews_sample.jsonl",
            Path(data_path) / "reviews.jsonl",
            Path(data_path) / "reviews.json",
            Path(data_path) / "reviews.csv"
        ]
        ...
        if review_file is None:
            raise FileNotFoundError(
                f"No reviews file found in {data_path}. "
                f"Expected one of: reviews_sample.jsonl, reviews.jsonl, reviews.json, reviews.csv"
            )

        print(f"Loading reviews from {review_file}")
        data_loader.load_reviews(str(review_file))

        data_loader.load_metadata()
        products_df = data_loader.merge_and_preprocess(min_reviews=2)
